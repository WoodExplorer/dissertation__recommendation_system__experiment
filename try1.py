...
def exp_learning_rate():                             # @@CURRENT!!!!!!!!!!!!!!!!!!!!!!!!
    """ exp: what will happen if learning_rate changes
    """
    #data_filename, delimiter = os.path.sep.join(['ml-latest-small', 'ratings.csv']), ','
    data_filename, delimiter, data_set = os.path.sep.join(['ml-1m', 'ratings.dat']), '::', '1M'
    #data_filename, delimiter = os.path.sep.join(['ml-10M100K', 'ratings.dat']), '::'
    ...
    N = 20  # TopN
    batch_words = 1000
    table_name_prefix = 'metrics__chap4_exp_X_learning_rate__N_%d___da_%s'
    table_name = table_name_prefix % (N, data_set)
    print 'table_name:', table_name
    ...
    #para_window_list = range(10, 100 + 1, 10)
    para_window_list = [5]
    para_learning_rate_list = np.array(range(1, 11)) * 0.005#[0.025]
    para_iter_list = [5]
    para_K_list = [10]
    para_topN_list = [20]
...
def exp_K():                             # @@CURRENT!!!!!!!!!!!!!!!!!!!!!!!!
    """ exp: what will happen if K (size of neighbor set) changes
    """
    #data_filename, delimiter = os.path.sep.join(['ml-latest-small', 'ratings.csv']), ','
    data_filename, delimiter, data_set = os.path.sep.join(['ml-1m', 'ratings.dat']), '::', '1M'
    #data_filename, delimiter = os.path.sep.join(['ml-10M100K', 'ratings.dat']), '::'
    ...
    N = 20  # TopN
    batch_words = 1000
    table_name_prefix = 'metrics__chap4_exp_X_K__N_%d___da_%s'
    table_name = table_name_prefix % (N, data_set)
    print 'table_name:', table_name
...
def time_overhead_CF(K = 10):
    ''' chap 4 exp 4 - time complexity - redo '''
    global arguments
    ...
    table_name_prefix = 'metrics__chap4_exp_X_time_overhead__CF__N_%d__da_%s'
    table_name = table_name_prefix % (N, data_set)
...
def main():
    ...
    if arguments['exp_learning_rate']:
        exp_learning_rate()
        return
